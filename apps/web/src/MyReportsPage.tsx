      ...
      <main className="container">
        ...
        {reports.length === 0 ? (
          ...
          <ul className="my-reports">
            {reports.map((r) => (
              <li key={r.number} className="my-reports__item">
                <Link
                  to={`/track?number=${encodeURIComponent(r.number)}`}
                  className="my-reports__link"
                >
                  <span className="my-reports__body">
                    {r.title && (
                      <span className="my-reports__title">{r.title}</span>
                    )}
                    <span className="my-reports__number">{r.number}</span>
                  </span>
